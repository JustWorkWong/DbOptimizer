...
internal interface IDatabaseMcpFallbackExecutor
{
 ...
 * ========================= */
internal sealed class DatabaseMcpFallbackExecutor(
    ...
    ILogger<DatabaseMcpFallbackExecutor> logger) : IDatabaseMcpFallbackExecutor
{
    ...
    {
        await using var connection = new MySqlConnection(runtimeOptions.ConnectionStrings.MySql);
        ...
        return toolKind switch
        {
            McpToolKind.Query => await ExecuteReaderAsync(
                ...
                command => command.CommandTimeout = runtimeOptions.Mcp.TimeoutSeconds,
                cancellationToken),
            McpToolKind.DescribeTable => await ExecuteReaderAsync(
                ...
                command =>
                {
                    command.CommandTimeout = runtimeOptions.Mcp.TimeoutSeconds;
    ...
    private async Task<string> ExecuteWithPostgreSqlAsync(
        ...
        CancellationToken cancellationToken)
    {
        await using var connection = new NpgsqlConnection(runtimeOptions.ConnectionStrings.PostgreSql);
        await connection.OpenAsync(cancellationToken);
